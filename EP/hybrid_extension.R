...
hyb = function(u_df, ep_params, D = ncol(u_df) - 1) {
    
    H_k = ep_params$H_k
    H_k_inv = ep_params$H_k_inv
    lambda_k = ep_params$lambda_k
    m_k = ep_params$m_k
    ...
    lambda_k = apply(psi_df[,1:D], 1, lambda, prior = prior)
    ...
    for (k in 1:K) {
        ...
        log_terms[k] = D / 2 * log(2 * pi) - 0.5 * log_det(H_k) - 
            psi_df$psi_u[k] + sum(lambda_k[,k] * u_k) - 
            0.5 * t(u_k) %*% H_k %*% u_k + 
