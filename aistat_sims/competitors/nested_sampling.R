...
logZ = -99999 # initial value for log marignal likelihood
...
while (j <= J * H) {
    ...
    H = exp(log_wt - logZ_new) * loglike_vec[worst] + 
    ...
    logL_star = loglike_vec[worst] # new likelihood constraint
    logL_new = logL_star
    # draw from prior until we find a prior to over-write the worst prior
    while (logL_new <= logLstar) {
